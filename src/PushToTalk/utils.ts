...
export function parseKeycode(entry: string): KeyButton | null {
  ...
  let newFormat = /^!!/.test(entry);
  if (newFormat) {
    return parseNewFormat(entry);
  }
...
function parseNewFormat(entry: string): KeyButton | null {

  // new format
  // !!ctrl+shift+m

  console.log("new format");
  let fullEntry = entry.substring(2).replace(/\(.+\)/, '').replace(/\*\d$/, '').split('+');
  console.log(fullEntry);

  if (fullEntry.length < 1) {
    console.log(`Could not find valid keys to press in entry: ${entry}`);
    log.error(`Could not find valid keys to press in entry: ${entry}`);
    return null;
  }
  ...
  let codes = fullEntry.map((c): Code => {
    return {
      key: c as KeyboardButton,
      code: 0,
      asScanCode: false
    };
  });
