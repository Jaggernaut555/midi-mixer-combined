...
export class HueSyncApi {
  ...
  private async _syncLights(): Promise<void> {
    ...
    const lightsRes: Hue.Lights = (await axios(`${baseUrl}/lights`).then(
      (res) => res.data
    )) as Hue.Lights;

    // const lightsRes: Hue.Lights = {};
  ...
  private async _syncGroups(): Promise<void> {
    ...
    const [groupsRes, scenesRes]: [Hue.Groups, Hue.Scenes] = await Promise.all([
      axios(`${baseUrl}/groups`).then((res) => res.data) as Promise<Hue.Groups>,
      axios(`${baseUrl}/scenes`).then((res) => res.data) as Promise<Hue.Scenes>,
    ]);
    ...
    Object.entries(groupsRes).forEach(([id, group]) => {
      if (!this.assignments.groups[id]) {
        ...
        assignment.on("volumeChanged", (volume: number) => {
          ...
          axios(`${baseUrl}/groups/${id}/action`, {
            method: "PUT",
            data: JSON.stringify(change),
          }).then((res) => {
            if (res.status == 200) {
