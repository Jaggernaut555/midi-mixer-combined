...
const cleanUpConnections = async () => {
  ...
  $MM.setSettingsStatus("DiscordStatus", "Disconnected");
};
...
export const initDiscordPlugin = async () => {
  /**
   * Disconnect any running instances.
   */
  await cleanUpConnections();

  $MM.setSettingsStatus("DiscordStatus", "Getting plugin settings...");
  const settings = await $MM.getSettings();

  const clientId = settings.DiscordClientId as string;
  const clientSecret = settings.DiscordClientSecret as string;
  const presence = settings.DiscordPresence as string;
...
$MM.onSettingsButtonPress("DiscordReconnect", initDiscordPlugin);
